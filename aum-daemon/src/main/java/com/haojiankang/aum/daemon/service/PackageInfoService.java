...
public class PackageInfoService {
    ...
    public void save(PackageInfo info, byte[] bytes ) throws  Exception{
        ...
        if(list==null||list.size()==0||!list.stream().anyMatch(p->p.getState()==null||p.getState()==true)){
            ...
            pkgRep.create(info);
            FileUtils.saveFile(bytes,getPackageDir(info.getAppcode(),info.getPointcode() ),info.getVersion());
        }
    }
    ...
    public void runUpdate(String appcode,String pointcode){
        ...
        StringBuilder pkgArg=new StringBuilder();
        pkgList.stream().forEach(pkg->{
            String name=pkg.getAppcode()+"_"+pkg.getPointcode()+"_"+pkg.getVersion();
            FileUtils.copyFile(getPackageDir(pkg.getAppcode(),pkg.getPointcode()),new File(PKG_TMP_DIR,name));
            pkgArg.append(name);
            pkgArg.append(",");
        });
        updateProgram(pkgArg.toString());
    }
    private void updateProgram(String args){
        ...
        try{
            Runtime.getRuntime().exec(String.format(UPGRADE_CMD,execFile.getAbsolutePath(),args));
        }catch (Exception e){
